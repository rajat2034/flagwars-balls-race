...
// Trigger alternate race events (football shower, gravity flip, speed surge, blackout, teleportation)
  ...
  render() {
    ...
    // Map theme backgrounds (skipped for jungle, which uses Amazon canopy image instead)
    ...
    if (this.track) {
      ...
      if (this.track.topPoints.length > 1) {
        ...
        // ===== TOP OF TRACK: Dense rainforest canopy wall [REMOVED - using Amazon BG image instead]
        if (this.currentThemeKey === 'jungle') {
          // no-op; background rendered via amazon canopy image
        }
            // [all old jungle background rendering removed]
          this.ctx.stroke();
          this.ctx.beginPath();
